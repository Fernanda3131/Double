...
function GestionPrendas() {
  // Carousel para fotos (como en DetallePrenda)
  ...
  // Fotos actuales (solo las que existen) - después de declarar 'form'
  const fotos = [form.foto_actual, form.foto2_actual, form.foto3_actual, form.foto4_actual].filter(Boolean);
  const hasMultipleFotos = fotos.length > 1;
  const goToNext = () => setCurrent((prev) => (prev === fotos.length - 1 ? 0 : prev + 1));
  const goToPrev = () => setCurrent((prev) => (prev === 0 ? fotos.length - 1 : prev - 1));
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <div className="editar-container" style={{position: 'relative', flex: '1 0 auto'}}>
        ...
        <div className="editar-panel">
          <div className="detalle-prenda-foto-col" style={{marginBottom: 24}}>
            {fotos.length > 0 ? (
              hasMultipleFotos ? (
                <div className="detalle-prenda-carrusel">
                  <div style={{ position: 'relative', display: 'inline-block' }} className="carrusel-hover-group">
                    <button
                    ...
                    <label style={{cursor:'pointer'}}>
                      ...
                      />
                      <input type="file" name={`foto${current === 0 ? '' : current+1}`} accept="image/*" onChange={handleChange} style={{display:'none'}} />
                    ...
                    <button
                      ...
                      type="button"
                      style={{right: 8, zIndex: 2, position: 'absolute', top: '50%', transform: 'translateY(-50%)'}}>
                      <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M8 5L14 11L8 17" stroke="#a07e44" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                      </svg>
                    </button>
                  </div>
                  <div className="detalle-prenda-carrusel-indicador" style={{textAlign: 'center', marginTop: 10}}>
                    {fotos.map((_, idx) => (
                      <span
                        key={idx}
                ...
                <label style={{cursor:'pointer'}}>
                  <img
                    ...
                    className="detalle-prenda-foto-grande"
                  />
                  <input type="file" name="foto" accept="image/*" onChange={handleChange} style={{display:'none'}} />
                  <div className="carrusel-hover-overlay">
                    Cambiar foto
                  </div>
                </label>
