...
function MiPerfil() {
  ...
  const cargarPerfilPorSesion = async () => {
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/api/mi_perfil?t=${Date.now()}`, {
        method: "GET",
        credentials: "include",
        cache: "no-cache",
        headers: {
                ...
                <div className="sidebar-contact-info">
                  ...
                  <div className="contact-item">
                    ...
                    {isEditMode ? (
                      ...
                      <span className="contact-text">Talla: {perfil.talla_usuario && perfil.talla_usuario.trim() !== '' ? perfil.talla_usuario : "No especificada"}</span>
                    )}
                  </div>
                  <div className="contact-item">
                    <div className="contact-icon">📅</div>
                    {isEditMode ? (
                      ...
                      <span className="contact-text">
                        Miembro desde {(() => {
                          // Si no hay fecha, mostrar "No disponible"
                          if (!perfil.creado_en) return "No disponible";
                          let fechaStr = perfil.creado_en;
                          // Si tiene espacio, tomar solo la fecha
                          if (typeof fechaStr === 'string' && fechaStr.includes(' ')) {
                            fechaStr = fechaStr.split(' ')[0];
                          }
                          // Si es tipo Date, usar directamente
                          let fecha;
                          if (fechaStr instanceof Date) {
                            ...
                            if (partes.length >= 2) {
                              // new Date(year, monthIndex) (mes 0-indexado)
                              fecha = new Date(parseInt(partes[0]), parseInt(partes[1]) - 1);
                            } else {
                              fecha = new Date(fechaStr);
                            }
                          }
                          if (isNaN(fecha.getTime())) return "No disponible";
                          // Formato completamente en español
