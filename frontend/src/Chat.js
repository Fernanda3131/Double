...
function Chat({ id_destinatario, destinatarioInfo, onBack, mensajeInicial }) {
    // Resetear primeraVez cada vez que cambia el destinatario (nuevo chat)
    useEffect(() => {
      setPrimeraVez(true);
    }, [id_destinatario]);
  ...
  // Scroll al final de los mensajes solo si el usuario está cerca del fondo, o es la primera vez (al abrir el chat)
  const scrollToBottom = () => {
    const container = mensajesContainerRef.current;
    if (!container) return;
    const threshold = 120; // px desde el fondo para considerar "cerca del final"
    const isNearBottom = container.scrollHeight - container.scrollTop - container.clientHeight < threshold;
    if (primeraVez) {
      // Siempre hacer scroll al fondo al abrir el chat
    ...
    // Si el usuario no está cerca del fondo y no es la primera vez, no hacemos scroll
  };
  ...
  useEffect(() => {
    if (mensajes.length > 0 && primeraVez) {
      scrollToBottom();
      setPrimeraVez(false);
