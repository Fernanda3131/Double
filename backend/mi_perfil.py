        ...
        with conexion.cursor() as cursor:
            cursor.execute(
                ...
                SELECT 
                    ...
                    u.banner_usuario AS banner_usuario,
                ...
                WHERE u.id_usuario = %s
                  AND (p.id_publicacion IS NULL OR EXISTS (SELECT 1 FROM publicacion pub2 WHERE pub2.id_publicacion = p.id_publicacion))
                """,
                (id_usuario,)
            )
            ...
            usuario = {
                ...
                'banner_usuario': perfil[0].get('banner_usuario'),
                'creado_en': creado_en,
            ...
            # Agregar prendas si existen, evitando duplicados por nombre_prenda y foto_prenda
            combinaciones_vistas = set()
            for row in perfil:
                if row['id_prenda'] is not None:
                    foto_prenda = row['foto_prenda'] if row['foto_prenda'] else 'default.jpg'
                    clave = (str(row['nombre_prenda']).strip().lower(), str(foto_prenda).strip().lower())
...
def api_mi_perfil():
    ...
    print(f"🔎 PERFIL DEVUELTO POR obtener_perfil({id_usuario}): {perfil}")
        ...
        try:
            with conexion.cursor() as cursor:
                ...
                cursor.execute("""
                    ...
                    WHERE usuario_valorador_id = %s AND usuario_valorado_id = %s
                ...
                if valoracion_existente:
                    ...
                    cursor.execute("""
                        UPDATE valoracion 
                        SET puntaje = %s, fecha_valoracion = NOW()
                        WHERE usuario_valorador_id = %s AND usuario_valorado_id = %s
                    ...
                    cursor.execute("""
                        ...
                        (usuario_valorador_id, usuario_valorado_id, puntaje, fecha_valoracion)
                        VALUES (%s, %s, %s, NOW())
